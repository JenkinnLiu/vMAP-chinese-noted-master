...
if __name__ == "__main__":
    #############################################
    # init config
    torch.backends.cudnn.enabled = True #确保每次返回的卷积算法是最快的
    ...
    # param for vis
    vis3d = open3d.visualization.Visualizer() #开启可视化窗口
    ...
    intrinsic_open3d = open3d.camera.PinholeCameraIntrinsic( #相机内参
        width=cfg.W,
        height=cfg.H,
        fx=cfg.fx,
        fy=cfg.fy,
    ...
    if not cfg.live_mode:
        ...
        dataloader_iterator = iter(dataloader) #DataLoader
    ...
    for frame_id in tqdm(range(dataset_len)): #遍历所有帧
        print("*********************************************")
        # get new frame data
        ...
        if sample is not None:  # 新的一帧
            last_frame_time = time.time() # 更新时间
            with performance_measure(f"Appending data"): #添加数据
                rgb = sample["image"].to(cfg.data_device) #加载rgb,depth,位姿到GPU
                depth = sample["depth"].to(cfg.data_device)
                twc = sample["T"].to(cfg.data_device)
                bbox_dict = sample["bbox_dict"]
                if "frame_id" in sample.keys():
                    live_frame_id = sample["frame_id"] #当前帧id
                else:
                    live_frame_id = frame_id
                if not cfg.live_mode:  # 不是实时可视化模式
                    inst = sample["obj"].to(cfg.data_device) # 加载目标实例分割到GPU
                    obj_ids = torch.unique(inst) #获取实例id
                else:
                    inst_data_dict = sample["obj"]
                    obj_ids = inst_data_dict.keys()
                # append new frame info to objs in current view，将当前帧的信息添加到当前视图中的物体中
                for obj_id in obj_ids:
                    if obj_id == -1:    # unsured area
                        continue
                    obj_id = int(obj_id)
                    # convert inst mask to state，将实例分割转换为状态
                    # 个人理解：将实例分割转换为state数组，里面利用下标mask来表示分割的物体，0表示像素不属于物体，1表示像素属于物体，2表示像素状态未知
                    if not cfg.live_mode:
                        state = torch.zeros_like(inst, dtype=torch.uint8, device=cfg.data_device) #初始化状态
                        state[inst == obj_id] = 1 # 1: obj,  2: unsure
                        state[inst == -1] = 2
                    else:
                        inst_mask = inst_data_dict[obj_id].permute(1,0) #转置
                        label_list = torch.unique(inst_mask).tolist() #获取实例id
                        state = torch.zeros_like(inst_mask, dtype=torch.uint8, device=cfg.data_device) # 初始化状态
                        state[inst_mask == obj_id] = 1
                        state[inst_mask == -1] = 2
                    bbox = bbox_dict[obj_id] #获取物体实例的锚框
                    if obj_id in vis_dict.keys(): #将vis_dict中的物体都添加进关键帧
                        scene_obj = vis_dict[obj_id]  # get obj
                        scene_obj.append_keyframe(rgb, depth, state, bbox, twc, live_frame_id) #添加关键帧
                    else: # init scene_obj
                        if len(obj_dict.keys()) >= cfg.max_n_models:
                            print("models full!!!! current num ", len(obj_dict.keys())) #模型满了
                            continue #直接下一帧
                        print("init new obj ", obj_id) #模型没满的话，初始化新的物体
                        if cfg.do_bg and obj_id == 0:   # todo param 将背景作为一个物体，放入Adam
                            ...
                            optimiser.add_param_group({"params": scene_bg.trainer.fc_occ_map.parameters(), "lr": cfg.learning_rate, "weight_decay": cfg.weight_decay}) #将背景的参数加入优化器
                            ...
                            scene_obj = sceneObject(cfg, obj_id, rgb, depth, state, bbox, twc, live_frame_id) #初始化物体
                            # scene_obj.init_obj_center(intrinsic_open3d, depth, state, twc)
                            obj_dict.update({obj_id: scene_obj}) #将物体加入字典
                            vis_dict.update({obj_id: scene_obj})
                            # params = [scene_obj.trainer.fc_occ_map.parameters(), scene_obj.trainer.pe.parameters()]
                            optimiser.add_param_group({"params": scene_obj.trainer.fc_occ_map.parameters(), "lr": cfg.learning_rate, "weight_decay": cfg.weight_decay}) #将物体的参数加入优化器
                            ...
                            if cfg.training_strategy == "vmap":
                                update_vmap_model = True #更新vmap模型
                                fc_models.append(obj_dict[obj_id].trainer.fc_occ_map)
                                pe_models.append(obj_dict[obj_id].trainer.pe) #将物体的模型加入vmap
        ...
        # dynamically add vmap 动态更新vmap
        with performance_measure(f"add vmap"):
            if cfg.training_strategy == "vmap" and update_vmap_model == True:
                fc_model, fc_param, fc_buffer = utils.update_vmap(fc_models, optimiser) #更新vmap
        ...
        with performance_measure(f"Sampling over {len(obj_dict.keys())} objects,"):
            if cfg.do_bg and scene_bg is not None: #如果有背景
                gt_rgb, gt_depth, valid_depth_mask, obj_mask, input_pcs, sampled_z \
                    = scene_bg.get_training_samples(cfg.n_iter_per_frame * cfg.win_size_bg, cfg.n_samples_per_frame_bg,
                                                    cam_info.rays_dir_cache) #获取背景的训练数据，得到背景的rgb,depth,有效深度掩码，物体掩码，输入点云，采样深度
                bg_gt_depth = gt_depth.reshape([gt_depth.shape[0] * gt_depth.shape[1]]) #背景的真实深度reshape一下
                ...
                bg_input_pcs = input_pcs.reshape(
                    [input_pcs.shape[0] * input_pcs.shape[1], input_pcs.shape[2], input_pcs.shape[3]]) #背景的输入点云reshape一下
                bg_sampled_z = sampled_z.reshape([sampled_z.shape[0] * sampled_z.shape[1], sampled_z.shape[2]]) #背景的采样深度reshape一下

            for obj_id, obj_k in obj_dict.items(): #遍历所有物体
                gt_rgb, gt_depth, valid_depth_mask, obj_mask, input_pcs, sampled_z \
                    = obj_k.get_training_samples(cfg.n_iter_per_frame * cfg.win_size, cfg.n_samples_per_frame,
                                                 cam_info.rays_dir_cache) #获取物体的训练数据，得到物体的真实rgb,真实depth,有效深度掩码，物体掩码，输入点云，采样深度
                # merge first two dims, sample_per_frame*num_per_frame
                # 先合并前两个维度，sample_per_frame*num_per_frame
                ...
                #最终得到的Batch_N_gt_depth是一个list，list中的每个元素是一个tensor，tensor的shape是[batch_size*num_per_frame]
                # Batch_N_gt_depth = [tensor1,tensor2,...]
                # 以此类推，准备好的训练数据有： 真实rgb，真实depth，有效深度掩码，物体掩码，输入点云，采样深度
        ...
        with performance_measure(f"Training over {len(obj_dict.keys())} objects,"): #报告一下训练的物体数
            for iter_step in range(cfg.n_iter_per_frame): #遍历每一帧的每一次迭代，共20次
                data_idx = slice(iter_step*n_sample_per_step, (iter_step+1)*n_sample_per_step) #每一次迭代的数据索引
                batch_input_pcs = Batch_N_input_pcs[:, data_idx, ...] #输入的点云
                batch_gt_depth = Batch_N_gt_depth[:, data_idx, ...] #真实深度
                batch_gt_rgb = Batch_N_gt_rgb[:, data_idx, ...] #真实rgb
                batch_depth_mask = Batch_N_depth_mask[:, data_idx, ...] #有效深度掩码
                batch_obj_mask = Batch_N_obj_mask[:, data_idx, ...] #物体掩码
                batch_sampled_z = Batch_N_sampled_z[:, data_idx, ...] #采样深度
                if cfg.training_strategy == "forloop": #这里是为了和vmap对比，forloop是直接for循环，vmap是并行计算
                    # for loop training
                    batch_alpha = []
                    batch_color = []
                    ...
                    # batched training
                    # 先优化pe，再优化fc，pe是点云的embedding，fc是embedding到alpha和color的映射，即fc是一个MLP
                    batch_embedding = vmap(pe_model)(pe_param, pe_buffer, batch_input_pcs) #并行计算，得到embedding
                    batch_alpha, batch_color = vmap(fc_model)(fc_param, fc_buffer, batch_embedding) #渲染，得到alpha和color，alpha为体密度
                    # vmap的原理是将for循环转换为并行计算，因此这里的batch_alpha和batch_color的shape是[batch_size,win_size*num_per_frame]
                    # vmap是torch的一个扩展库，可以将for循环转换为并行计算，加速计算
            ...
            # with performance_measure(f"Batch LOSS"):
                batch_loss, _ = loss.step_batch_loss(batch_alpha, batch_color,
                                     ...
                                     batch_sampled_z.detach()) #计算三个loss，alpha(occupancy) loss，color loss，depth loss
                ...
                if cfg.do_bg:
                    ...
                    batch_loss += bg_loss #将背景的loss加入到物体的loss中
            ...
            # with performance_measure(f"Backward"):
                if AMP:
                    ...
                    optimiser.step() #优化器更新
                optimiser.zero_grad(set_to_none=True) #梯度清零
                ...
                with torch.no_grad():
                    for model_id, (obj_id, obj_k) in enumerate(obj_dict.items()):
                        for i, param in enumerate(obj_k.trainer.fc_occ_map.parameters()):
                            param.copy_(fc_param[i][model_id]) #更新fc_param的每个小模型的参数
                        for i, param in enumerate(obj_k.trainer.pe.parameters()):
                            param.copy_(pe_param[i][model_id]) #更新pe_param的每个小模型的参数


        ####################################################################
        # live vis mesh，实时可视化网格
        if (((frame_id % cfg.n_vis_iter) == 0 or frame_id == dataset_len-1) or
            (cfg.live_mode and time.time()-last_frame_time>cfg.keep_live_time)) and frame_id >= 10: #每隔n_vis_iter帧可视化一次，或者是最后一帧，或者是实时模式下，超过keep_live_time的时间，更新可视化一次
            vis3d.clear_geometries()
            for obj_id, obj_k in vis_dict.items():
                bound = obj_k.get_bound(intrinsic_open3d) #获取物体的边界
                if bound is None:
                    print("get bound failed obj ", obj_id)
                    continue
                adaptive_grid_dim = int(np.minimum(np.max(bound.extent)//cfg.live_voxel_size+1, cfg.grid_dim)) #自适应的网格维度
                mesh = obj_k.trainer.meshing(bound, obj_k.obj_center, grid_dim=adaptive_grid_dim) #建立网格
                if mesh is None:
        ...
        with performance_measure("saving ckpt"): #保存checkpoint到log_dir
